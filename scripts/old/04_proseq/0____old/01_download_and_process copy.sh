...
# MEMORY:  ~8 GB peak (bowtie2 alignment)
#
# USAGE:
...
for SAMPLE_NAME in "${!SAMPLES[@]}"; do
    ...
    # Sanity check
    RAW_READ_COUNT=$(awk 'END{print NR/4}' "${FASTQ_INPUT}")
    ...
    TRIMMED_READ_COUNT=$(zcat "${TRIMMED_FASTQ}" | awk 'END{print NR/4}')
    ...
    if [[ -f "${NOSPIKEIN_FASTQ}" ]]; then
        ...
        NOSPIKEIN_FASTQ_UNZIPPED="${TRIMMED_DIR}/${SAMPLE_NAME}_nospikein.fastq"
        ...
        pigz -p "${THREADS}" "${NOSPIKEIN_FASTQ_UNZIPPED}"
        echo "[INFO] Spike-in filtering complete."
    fi

    # Count reads surviving spike-in filter
    NOSPIKEIN_READ_COUNT=$(zcat "${NOSPIKEIN_FASTQ}" | awk 'END{print NR/4}')
    ...
    if [[ -f "${RAW_BAM}" ]]; then
        ...
        echo "[INFO] Settings: --very-sensitive, MAPQ filtering done in next step"
    ...
    if [[ -f "${FILTERED_BAM}" ]]; then
        ...
        #
        # samtools sort: sort by coordinate (required for picard dedup and indexing)
