...
#   4b. (OPTIONAL) 5' end deduplication — collapse reads at same 5' position
...
#
#   STEP 4b — OPTIONAL 5' END DEDUPLICATION (DEDUP_5PRIME=true):
#     When enabled, uses samtools markdup to collapse reads sharing the same
#     5' mapping position and strand. Among a group of duplicates, the read
#     with the highest sum of base qualities is KEPT; all others are removed.
#
#     *** USE WITH CAUTION ***
...
#     This is why dedup is OFF by default (Mahat et al. 2016, proseq2.0).
#     Enable only if you suspect PCR duplicates from low-complexity libraries.
#
#     All deduplicated outputs carry a "_5prime_dedup" suffix so they NEVER
#     overwrite the standard (non-dedup) outputs.
#
# PARALLEL EXECUTION:
...
#
#   # With optional dedup:
#   DEDUP_5PRIME=true sbatch --array=0-3 ... 03_align_filter.sh
# =============================================================================
...
if [[ "${DEDUP_5PRIME}" == "true" ]]; then
    echo "[INFO] 5' end deduplication: ENABLED"
    echo "[INFO] Dedup outputs will use suffix: ${DEDUP_SUFFIX}"
else
...
for SAMPLE in "${RUN_SAMPLES[@]}"; do
    ...
    # =========================================================================
    # When enabled (DEDUP_5PRIME=true), collapses reads sharing the same
    # 5' unclipped mapping position and strand. Among each group of duplicates,
    # samtools markdup KEEPS the read with the highest sum of base qualities
    # and REMOVES all others.
    #
    # This is analogous to what Picard MarkDuplicates does for SE data, but
    # implemented natively in samtools (no Java dependency).
    #
    # Output carries the "_5prime_dedup" suffix to avoid overwriting the
    # standard _final.bam.
    # =========================================================================
    if [[ "${DEDUP_5PRIME}" == "true" ]]; then
        echo ""
        echo "[STEP 4b] Optional: 5' end deduplication..."
        echo "[NOTE] Among reads at the same 5' position + strand, the read with"
        echo "       the highest base-quality sum is KEPT; all others are removed."
        echo "[NOTE] Use with caution — at strong Pol II pause sites, identical"
        echo "       positions are EXPECTED biological signal, not PCR artifacts."

        DEDUP_BAM="${BAM_MM39_DIR}/${SAMPLE}_final${DEDUP_SUFFIX}.bam"
        DEDUP_STATS="${LOG_DIR}/${SAMPLE}_markdup_stats.txt"

        if [[ -f "${DEDUP_BAM}" ]]; then
            echo "[INFO] Dedup BAM already exists: ${DEDUP_BAM}. Skipping."
        else
            # samtools markdup for SE data:
            #   Groups reads by (5' unclipped start coordinate, strand).
            #   Within each group, keeps the read with the highest sum of
            #   base qualities. All other reads in the group are removed (-r).
            #
            # Flags:
            #   -r : Remove duplicates (don't just flag them)
            #   -s : Print duplicate statistics to stderr
            #
            # Input MUST be coordinate-sorted (our _final.bam already is).
            ...
            echo ""
            echo "[REMINDER] The removed reads were collapsed by 5' position + strand."
            echo "  For each group of duplicates, the read with the HIGHEST base-quality"
            echo "  sum was retained. This is the samtools markdup default for SE data."

            # Per-sample dedup QC file
            echo -e "${SAMPLE}\t${BEFORE_DEDUP}\t${AFTER_DEDUP}\t${REMOVED_DEDUP}\t${PCT_DUP}" \
                > "${QC_DIR}/dedup_stats_${SAMPLE}.tsv"
        fi
    fi

    # =========================================================================
    # Collect final QC stats (always based on non-dedup _final.bam)
    # =========================================================================
...
if [[ "${DEDUP_5PRIME}" == "true" ]]; then
    echo "  Dedup BAMs:  ${BAM_MM39_DIR}/*_final${DEDUP_SUFFIX}.bam"
    echo "  Dedup stats: ${QC_DIR}/dedup_stats_*.tsv"
fi
echo "  Stats:       ${QC_DIR}/alignment_stats_*.tsv"
echo ""
echo "Next: Run 04_make_bigwigs.sh (after all step 03 array tasks finish)"
if [[ "${DEDUP_5PRIME}" == "true" ]]; then
    echo "  Remember to also pass DEDUP_5PRIME=true to steps 04 and 05."
fi
