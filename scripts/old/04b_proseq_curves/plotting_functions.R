...
AXIS_LABEL_SIZE        <- 17
AXIS_TICK_SIZE         <- 15
RNAPII_LABEL_SIZE      <- 15
SNP_STATS_SIZE         <- 12
INSTANCE_COUNT_SIZE    <- 14
LEGEND_TEXT_SIZE        <- 15
LEGEND_TITLE_SIZE      <- 16

ARROW_HEAD_LENGTH_CM   <- 0.4
ARROW_LINE_WIDTH_MM    <- 0.8
ARROW_LINE_LENGTH_FRAC <- 0.15   # shorter arrow

# Maximum characters per line for TF subtitle wrapping
...
# =============================================================================
# HELPER: Wrap long text to fit within figure width
# =============================================================================

wrap_subtitle <- function(text, width = SUBTITLE_WRAP_WIDTH) {
  if (is.null(text) || nchar(text) == 0) return(text)
  # Split on commas (TF names are comma-separated), then greedily wrap
  words <- trimws(strsplit(text, ",\\s*")[[1]])
  lines <- character(0)
  current_line <- words[1]
  if (length(words) > 1) {
    for (i in 2:length(words)) {
      candidate <- paste0(current_line, ", ", words[i])
...
make_encounter_plot <- function(
  ...
  sub_line1 <- paste0(filter_label, "  \u2014  ", bw_label)
  sub_text <- if (nchar(tf_subtitle) > 0) {
    paste0(sub_line1, "\n", wrap_subtitle(tf_subtitle))
  } else {
    sub_line1
  }
  ...
  combined <- combined +
    plot_annotation(
      ...
      theme    = theme(
        ...
        plot.subtitle = element_text(size = SUBTITLE_FONT_SIZE, hjust = 0.5,
...
make_overlay_encounter <- function(
  ...
  sub_line1 <- paste0(overlay_label, "  \u2014  ", bw_label)
  sub_text <- if (nchar(tf_subtitle) > 0) {
    paste0(sub_line1, "\n", wrap_subtitle(tf_subtitle))
  } else {
    sub_line1
