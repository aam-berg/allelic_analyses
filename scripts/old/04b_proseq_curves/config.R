...
get_bigwig_sets <- function() {
  ...
  # Merged dedup bigwigs (5'-dedup versions)
  for (end_type in c("3prime", "5prime")) {
    sets[[paste0("merged_", end_type, "_dedup")]] <- list(
      plus  = file.path(bw_dir, "merged", sprintf("WT_PROseq_merged_5prime_dedup_%s_plus.bw",  end_type)),
      minus = file.path(bw_dir, "merged", sprintf("WT_PROseq_merged_5prime_dedup_%s_minus.bw", end_type)),
      label = paste0("merged_", end_type, "_dedup")
    )
  }
...
# ----- Plot aesthetics -------------------------------------------------------
PLOT_WIDTH       <- 10    # inches (narrower)
PLOT_HEIGHT      <- 13    # inches
PLOT_DPI         <- 200
